...
ui <- fluidPage(
  ...
  fluidRow(column(12,textInput("scholarID2", "Google Scholar ID", 
                               value = "CvhanwUAAAAJ", width = NULL, placeholder = NULL))),
  fluidRow(column(12,
...
server <- function(input, output) {
  ...
  pubs <- reactive({
    scholar.ID <- input$scholarID2  
    pubs=scholar::get_publications(id=scholar.ID)
    pubs
  })
  
  output$citations <- renderPrint({
    pubs<- pubs()
    pubs$citation<-paste("<p>",pubs$author, paste0("(", pubs$year, ")"), pubs$title, "<i>", pubs$journal, "</i>", pubs$number, "Citation Count = ", pubs$cites, "<p>")
    cat(pubs$citation)
  })
  
  output$name <- renderPrint({scholar.ID <- input$scholarID2  
  profile=scholar::get_profile(id=scholar.ID)
  cat(profile$name)
  })
